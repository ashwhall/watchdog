...
def scrape_petrescue():
    ...
    scrape_generic(
        'https://www.petrescue.com.au/listings/search/dogs?interstate=false&page=1&per_page=500&size%5B%5D=10&state_id%5B%5D=2')
    print(f'done - {db.count() - start_count} dogs scraped.', flush=True)
...
def scrape_adoptapet(driver):
    ...
    modal = selenium_get_with_wait(
        driver, lambda d: d.find_element_by_class_name('tingle-modal--visible'))
    if modal:
        close_btn = selenium_get_with_wait(
            driver, lambda d: d.find_element_by_class_name('tingle-btn--primary'))
        if close_btn:
            close_btn.click()

    advanced_search_btn = selenium_get_with_wait(
        driver, lambda d: d.find_element_by_class_name('advanced-search'))
    if advanced_search_btn:
        advanced_search_btn.click()

    # Form details
    anim_type_select = selenium_get_with_wait(
        driver, lambda d: d.find_element_by_id('animal-type'))
    if anim_type_select:
        Select(anim_type_select).select_by_value(
            'custom-mapping-1')  # Dog and puppy

    state_select = selenium_get_with_wait(
        driver, lambda d: d.find_element_by_id('state'))
    if state_select:
        Select(state_select).select_by_value('3')  # Victoria

    # Submission
    time.sleep(2)
    search_btn = selenium_get_with_wait(driver, lambda d: d.find_element_by_xpath(
        '//*[@id="search-button-bott"]/button'))
    if search_btn:
        search_btn.click()
...
def scrape_saveadog(driver):
    ...
    for category in ('small-dogs', 'puppies'):
        ...
        modal_close = selenium_get_with_wait(
            driver, lambda d: d.find_elements_by_class_name('mmodal__close'))
        if modal_close:
            selenium_try_click(modal_close[0])

        dog_boxes = selenium_get_with_wait(
            driver, lambda d: d.find_elements_by_class_name('animal-box-inner'))
        for dog_box in dog_boxes:
            img_src = dog_box.find_element_by_tag_name(
...
def scrape_rspca(driver):
    ...
    while True:
        driver.get(
            f'https://rspcavic.org/adoption/Search/?animal=Dog&location=&keywords=&seed=9&page={page}')

        modal_close = selenium_get_with_wait(
            driver, lambda d: d.find_elements_by_id('danqam'))
        if modal_close:
            selenium_try_click(modal_close[0])

        dog_boxes = selenium_get_with_wait(
            driver, lambda d: d.find_elements_by_class_name('animalSearchImageWrapper'))
        if len(dog_boxes) == 0:
            break
        for dog_box in dog_boxes:
            img_src = dog_box.find_element_by_tag_name(
...
def scrape_petbarn(driver):
    ...
    show_filter_btn = selenium_get_with_wait(
        driver, lambda d: d.find_element_by_id('showfilters'))
    if show_filter_btn:
        selenium_try_click(show_filter_btn)

    # Filters
    state_select_div = selenium_get_with_wait(
        driver, lambda d: d.find_element_by_class_name('sl-state'))
    state_select = None
    if state_select_div:
    ...
    for chkbox_id in ('lifestage-puppy', 'lifestage-adult', 'breed-size-sm'):
        chkbox_candidates = selenium_get_with_wait(
            driver, lambda d: d.find_elements_by_id(chkbox_id))
        for chkbox in chkbox_candidates:
            if chkbox.tag_name == 'input':
                selenium_try_click(chkbox)

    # Apply filter
    apply_btn = selenium_get_with_wait(
        driver, lambda d: d.find_element_by_id('filter-results'))
    if apply_btn:
        apply_btn.click()
    ...
    while more_pages:
        more_pages = False
        dog_boxes = selenium_get_with_wait(
            driver, lambda d: d.find_elements_by_class_name('sl-candidate'))
        for dog in dog_boxes:
            ...
            button = selenium_get_with_wait(
                driver, lambda d: d.find_element_by_class_name('enquire-now'))
            if button:
            ...
            close_btn = selenium_get_with_wait(
                driver, lambda d: d.find_element_by_class_name('sl-candidate-close'))
            if close_btn:
                close_btn.click()
        pagination_div = selenium_get_with_wait(
            driver, lambda d: d.find_element_by_class_name('sl-pagination'))
        if pagination_div:
...
def _facebook_login(driver):
    email_input = selenium_get_with_wait(
        driver, lambda d: d.find_element_by_id('m_login_email'))
    if email_input:
        pass_input = selenium_get_with_wait(
            driver, lambda d: d.find_element_by_id('m_login_password'))
        if pass_input:
            submit_btn = selenium_get_with_wait(
                driver, lambda d: d.find_element_by_class_name('_56bu'))
            if submit_btn:
                email_input.send_keys(EMAIL)
                pass_input.send_keys(PWD)
                submit_btn.click()

    not_now_btn = selenium_get_with_wait(
        driver, lambda d: d.find_element_by_id('_56bw'))
    if not_now_btn:
        not_now_btn.click()
...
def get_img_src(img):
    ...
    try:
        ...
        img_src = css_img_prop[css_img_prop.index(
            '"') + 1:css_img_prop.rindex('"')]
    except:
        pass
    return img_src
...
def _scrape_fb(driver, url):
    driver.get(url)
    selenium_get_with_wait(
        driver, lambda d: d.find_elements_by_class_name('_78cz'))
    ...
    for link_div in link_divs:
        ...
        if links := link_div.find_elements_by_tag_name('a'):
            href = links[0].get_attribute('href').replace(
                'm.facebook', 'facebook').replace('touch.facebook', 'facebook')
...
def scrape(headless=False):
    ...
    driver = webdriver.Chrome(
        executable_path='chromedriver_linux64/chromedriver', options=chrome_options)
